...
def enterParticpantsScreen():
    while True:
        print("WELCOME TO THE PARTICIPANTS PAGE")
...
def displayFestDetails(choice):
    try:
        ...
        print(tabulate(data, headers=['Fest Name','Start Date','End Date','Description','Venue','Main Sponsor'], tablefmt='psql'))
    except:
        print("Some error occurred")
...
def displayEventNamesAndTakeChoice():
    try:
        ...
        print(tabulate(data, headers=['Event Id' ,'Event Name'], tablefmt='psql'))
...
def displayEventDetails(choice):
    try:
        ...
        cursor.execute(query)
        data = cursor.fetchall()
        print(tabulate(data, headers=['Event Name','Club','Event Head','Event Date','Start Time','End Time','Venue','Winner'], tablefmt='psql'))
